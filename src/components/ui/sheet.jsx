...
const SheetContent = React.forwardRef(({ className, children, side = 'right', ...props }, ref) => (
  <SheetPortal>
    ...
    >
      {children}
      <SheetPrimitive.Close className="absolute right-4 top-4 rounded-sm text-cream opacity-70 transition-opacity hover:text-gold hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-gold">
        <X className="size-5" />
